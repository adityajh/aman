...
function orsSequence(client: { sessions: { scheduledAt: Date | string; note: { orsTotal: string | null } | null }[] }): OrsPoint[] {
...
export async function GET(
  ...
  try {
    ...
    const insufficient = (reason: string, extra: Record<string, unknown> = {}) =>
      NextResponse.json({
    ...
    if (!current) return new NextResponse("Client not found", { status: 404 });
    ...
    const trajectory = currentSeq.map((point, i) => {
      ...
      const avg = hasBand
        ? valuesAtI.reduce((a, b) => a + b, 0) / cohortN
        : null;
      return {
        session: i + 1,
        date: point.date,
    ...
    if (comparable.length === 0) {
      return insufficient(
        ...
        { cohortSize: cohort.length, currentInitialOrs, initialOrsBand: [bandLow, bandHigh], trajectory }
      );
    }
