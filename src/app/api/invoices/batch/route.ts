...
export async function POST(req: Request) {
  ...
  return await withTenantContext(tenantId, async (tx) => {
    try {
      ...
      const settingsRow = await tx.query.practiceSettings.findFirst();
      const defaultDueDays = settingsRow?.invoiceDueDays ?? 15;
      const nDays =
        Number.isFinite(Number(dueDays)) && Number(dueDays) >= 0
          ? Number(dueDays)
          : defaultDueDays;
      ...
      for (const clientId of clientIds) {
        try {
          ...
          const candidateSessions = await tx.query.sessions.findMany({
            where: and(
          ...
          const billable = candidateSessions
            .map((s: any) => {
              const amount =
                s.status === "completed"
          ...
          if (client?.defaultFeeSchemeId) {
            const scheme = await tx.query.feeSchemes.findFirst({
          ...
          // 4. Totals
          const subtotal = billable.reduce((sum: any, b: any) => sum + b.amount, 0);
          ...
          await applyClientCredit(tx, tenantId, clientId, currency, newInvoice);
