...
export async function GET() {
  ...
  return await withTenantContext(tenantId, async (tx) => {
    try {
      ...
      const closedClients = await tx.query.clients.findMany({
      ...
      const activeSessions = await tx.query.sessions.findMany({
        ...
        orderBy: (s: any, { desc }: any) => [desc(s.scheduledAt)],
      });
      ...
      const clientData = closedClients.map((c: any) => {
        const withOrs = c.sessions.filter((s: any) => s.note?.orsTotal != null);
        const initialOrs =
          withOrs.length > 0 ? parseFloat(withOrs[0].note!.orsTotal!) : null;
        const finalOrs =
          withOrs.length > 0
            ? parseFloat(withOrs[withOrs.length - 1].note!.orsTotal!)
            : null;
        ...
        const srsScores = c.sessions
          ...
          .map((s: any) => parseFloat(s.note!.srsTotal!));
      ...
      // % Initial ORS ≤ cutoff
      const lowOrsCohort = clientData.filter(
        (c: any) => c.initialOrs != null && c.initialOrs <= orsCutoff,
      );
      ...
      const medianTenureWeeks = median(
        clientData
          .filter((c: any) => c.tenureWeeks != null)
          .map((c: any) => c.tenureWeeks!),
      );
      const medianSessions = median(clientData.map((c: any) => c.sessionCount));

      // Outcome ratios — cohort: low-ORS clients with both initial + final ORS recorded
      const outcomeCohort = lowOrsCohort.filter(
        (c: any) => c.initialOrs != null && c.finalOrs != null,
      );
      ...
      const pctDeterioration =
        outcomeDenom > 0 ? (deteriorationCount / outcomeDenom) * 100 : null;
      const pctNoChange =
        outcomeDenom > 0 ? (noChangeCount / outcomeDenom) * 100 : null;
      const pctCsc = outcomeDenom > 0 ? (cscCount / outcomeDenom) * 100 : null;
      const ptrIRate =
        outcomeDenom > 0 ? (ptrICount / outcomeDenom) * 100 : null;
      ...
      const highOrsUnflagged = highOrsCohort.filter(
        (c: any) => c.prematureTerminationManual == null,
      ).length;
      const ptrIIManualCount = highOrsCohort.filter(
        (c: any) => c.prematureTerminationManual === true,
      ).length;
      const finalPtrRate =
        totalClosed > 0
          ? ((ptrICount + ptrIIManualCount) / totalClosed) * 100
          : null;

      // Average SRS (all sessions of all closed clients)
      const allSrs = clientData.flatMap((c: any) => c.srsScores);
      const avgSrs =
        allSrs.length > 0
          ? allSrs.reduce((a: any, b: any) => a + b, 0) / allSrs.length
          : null;
      ...
      const avgInitial =
        bothOrsCohort.length > 0
          ? bothOrsCohort.reduce((s: any, c: any) => s + c.initialOrs!, 0) /
            bothOrsCohort.length
          : null;
      const avgFinal =
        bothOrsCohort.length > 0
          ? bothOrsCohort.reduce((s: any, c: any) => s + c.finalOrs!, 0) /
            bothOrsCohort.length
          : null;
      const sdInitial = stddev(bothOrsCohort.map((c: any) => c.initialOrs!));
      const effectSize =
        avgInitial != null &&
        avgFinal != null &&
        sdInitial != null &&
