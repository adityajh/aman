  ...
  return await withTenantContext(tenantId, async (tx) => {
    try {
      ...
      for (const s of allSessions) {
        if (!clientSessionMap.has(s.clientId))
          clientSessionMap.set(s.clientId, []);
      ...
      const enriched = allSessions.map((s: any) => {
        ...
        const idx = clientHistory.findIndex((h: any) => h.id === s.id);

        const currentOrs =
          s.note?.orsTotal != null ? Number(s.note.orsTotal) : null;
        const currentSrs =
          s.note?.srsTotal != null ? Number(s.note.srsTotal) : null;

        // Initial ORS = first session in history with an ORS score
        const firstWithOrs = clientHistory.find(
          (h: any) => h.note?.orsTotal != null,
        );
        const initialOrs =
          firstWithOrs?.note?.orsTotal != null
            ? Number(firstWithOrs.note.orsTotal)
            : null;

        // Previous session (chronologically one before this one)
        const prevSession = idx > 0 ? clientHistory[idx - 1] : null;
        const prevOrs =
          prevSession?.note?.orsTotal != null
            ? Number(prevSession.note.orsTotal)
            : null;
        const prevSrs =
          prevSession?.note?.srsTotal != null
            ? Number(prevSession.note.srsTotal)
            : null;
        ...
        if (currentOrs !== null) {
          const sessionChange =
            initialOrs !== null ? currentOrs - initialOrs : null;
          ...
          if (isAboveCutoff && isRCI) {
            ...
            initialOrs !== null &&
            initialOrs - currentOrs > (settings?.orsDeteriorationThreshold ?? 5)
          ) {
        ...
        if (currentSrs !== null) {
          ...
          if (
            currentSrs < srsCutoff ||
            (dropFromPrev !== null &&
              dropFromPrev > (settings?.srsDeclineThreshold ?? 2))
          ) {
    ...
    try {
      ...
      const rows = Array.from({ length: totalCount }, (_, i) => {
        ...
        return {
          ...
          modality,
          feeCharged:
            feeCharged && feeCharged !== "" ? feeCharged.toString() : undefined,
          feeSchemeId:
            feeSchemeId && feeSchemeId !== "" ? feeSchemeId : undefined,
        };
      });

      const inserted = await tx.insert(sessions).values(rows).returning();
